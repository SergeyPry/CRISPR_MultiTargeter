...
<title>Find common and unique CRISPR guide RNA targets for similar genes or transcripts</title>
...
<body id="main_body" style="background-color: #C9D0E8;">
...
def highlight_targets_output(sequences_targets_forw, sequences_targets_rev): 
	...
	<h3>Common and unique CRISPR sgRNA targets in multiple genes or transcripts</h3>
	<hr style="height:3px;border-width:4; width: 1120px; color:gray;background-color:gray">
	"""
	# print a list of input identifiers
	print """
       <h4> The identifiers for input sequences are: <br />
	"""
	...
	for id in sorted(sequences_targets_forw.keys()):
		if id != "consensus":
			...
			if species in animals:
				print("<h4><a style='color: #0000FF;' href='http://useast.ensembl.org/%s_%s/Gene/Summary?db=core;g=%s'>%s Ensembl Link</a></h4>" %(genus, sp, id, id))
			elif species in plants:
				print("<h4><a style='color: #0000FF;' href='http://plants.ensembl.org/Multi/Search/Results?species=all;idx=;q=%s;site=ensemblunit'>%s EnsemblPlants Link</a></h4>" %(id, id))
				...
				if "AT" in seq:
					...
					print("<h4><a style='color: #0000FF;' href='http://useast.ensembl.org/Gene/Summary?db=core;g=%s'>%s Ensembl Link</a></h4>" %(id, id))
			#####################
			# END of OUTPUT
			#####################   
	...
	# print the first line of markers to initialize the process
	print_markers(start_coords, curr_slice)
	...
	for targ_id in reverse_targets.keys():
		...
		if reverse_targets[targ_id]["coord_target"][0] < curr_slice:
			...
			if reverse_targets[targ_id]["coord_target"][1] < curr_slice:
				...
				# print markers if there are any in this segment
				print_markers(start_coords, curr_slice)
...
def targets_table_output(sequences_targets_forw, sequences_targets_rev):
	...
	if len(sequences_targets_forw["consensus"]["aligned_matches"]) == 0:
		...
		# print the <div> element defining the pretty table style
		print('<div class="CSSTableGenerator" style="width:1100px; margin:10px;">')
		
		# header for the forward strand of individual sequence data
		print """
				<table >
					<tr>
						<td>
		...
		for targ_id in sorted(sequences_targets_forw["consensus"]["aligned_matches"].keys()):
			for seq_key in sequences_targets_forw.keys():
				if seq_key != "consensus":
					if "exon_number" in sequences_targets_forw[seq_key]["orig_matches"][targ_id].keys():
		...
		for targ_id in sorted(sequences_targets_forw["consensus"]["aligned_matches"].keys()):
			...
			for seq_key in sequences_targets_forw.keys():
				if seq_key != "consensus":
					...
					print('<td>%d</td>' %targ_id)
					print('<td>%s</td>' %sequences_targets_forw[seq_key]["orig_matches"][targ_id]["target_seq"])
					...
					print('<td>')
					print "{0:2.2f}".format(GCpercent(sequences_targets_forw[seq_key]["orig_matches"][targ_id]["target_seq"]))
					print('</td>')

					print('<td>')
					print "{0:2.2f}".format(DNA_RNA_Tm(sequences_targets_forw[seq_key]["orig_matches"][targ_id]["target_seq"]))
					print('</td>')
					
					if "exon_number" in sequences_targets_forw[seq_key]["orig_matches"][targ_id].keys():
						print('<td>%s</td>' %sequences_targets_forw[seq_key]["orig_matches"][targ_id]["exon_number"])
		...
		for targ_id in sorted(sequences_targets_forw["consensus"]["aligned_matches"].keys()):
			...
			for seq_key in sequences_targets_forw.keys():
				if seq_key != "consensus":
					...
					sys.stdout.write('\t')
					sys.stdout.write('%4d' %(sequences_targets_forw[seq_key]["orig_matches"][targ_id]["coord_target"][1] + 1))
					sys.stdout.write('\t')
					sys.stdout.write("{0:2.2f}".format(GCpercent(sequences_targets_forw[seq_key]["aligned_matches"][targ_id]["target_seq"])))
					sys.stdout.write('\t')
					sys.stdout.write("{0:2.2f}".format(DNA_RNA_Tm(sequences_targets_forw[seq_key]["aligned_matches"][targ_id]["target_seq"])))
					
					if "exon_number" in sequences_targets_forw[seq_key]["orig_matches"][targ_id].keys():
						sys.stdout.write('\t')
						sys.stdout.write('%20s' %sequences_targets_forw[seq_key]["orig_matches"][targ_id]["exon_number"])
	...
	if len(sequences_targets_rev["consensus"]["aligned_matches"]) == 0:
		...
		# header for the reverse strand individual sequence target data
		print """
				<table >
					<tr>
						<td>
		...
		for targ_id in sorted(sequences_targets_rev["consensus"]["aligned_matches"].keys()):
			for seq_key in sequences_targets_rev.keys():
				if seq_key != "consensus":
					if "exon_number" in sequences_targets_rev[seq_key]["orig_matches"][targ_id].keys():
						exon_info_available = True
		...
		for targ_id in sorted(sequences_targets_rev["consensus"]["aligned_matches"].keys()):
			...
			for seq_key in sequences_targets_rev.keys():
				if seq_key != "consensus":
					...
					print('<td>%s</td>' %seq_key)
					print('<td>%d</td>' %targ_id)
					print('<td>%s</td>' %sequences_targets_rev[seq_key]["orig_matches"][targ_id]["target_seq"])
					...
					print('<td>%d</td>' %(sequences_targets_rev[seq_key]["orig_matches"][targ_id]["coord_target"][0] + 1))
					print('<td>%d</td>' %(sequences_targets_rev[seq_key]["orig_matches"][targ_id]["coord_target"][1] + 1))

					print('<td>')
					print "{0:2.2f}".format(GCpercent(sequences_targets_rev[seq_key]["orig_matches"][targ_id]["target_seq"]))
					print('</td>')

					print('<td>')
					print "{0:2.2f}".format(DNA_RNA_Tm(sequences_targets_rev[seq_key]["orig_matches"][targ_id]["target_seq"]))
					print('</td>')
					
					if "exon_number" in sequences_targets_rev[seq_key]["orig_matches"][targ_id].keys():
						print('<td>%s</td>' %sequences_targets_rev[seq_key]["orig_matches"][targ_id]["exon_number"])

					if (pam_seq == "NGG") and (length == 20):
						if "seq_forscore" in sequences_targets_rev[seq_key]["aligned_matches"][targ_id]:
							print('<td>')
							print "{0:3.2f}".format(calcDoenchScore(sequences_targets_rev[seq_key]["aligned_matches"][targ_id]["seq_forscore"]))
							print('</td>')
						else:
							print('<td>%s</td>' % "")
		...
		for targ_id in sorted(sequences_targets_rev["consensus"]["aligned_matches"].keys()):
			...
			for seq_key in sequences_targets_rev.keys():
				if seq_key != "consensus":
					...
					sys.stdout.write('%3d' %targ_id)
					sys.stdout.write('\t')
					sys.stdout.write('%25s' %sequences_targets_rev[seq_key]["aligned_matches"][targ_id]["target_seq"])
					sys.stdout.write('\t')
					...
					sys.stdout.write('%5d' %(sequences_targets_rev[seq_key]["orig_matches"][targ_id]["coord_target"][0] + 1))
					sys.stdout.write('\t')
					sys.stdout.write('%4d' %(sequences_targets_rev[seq_key]["orig_matches"][targ_id]["coord_target"][1] + 1))
					sys.stdout.write('\t')
					sys.stdout.write("{0:2.2f}".format(GCpercent(sequences_targets_rev[seq_key]["aligned_matches"][targ_id]["target_seq"])))
					sys.stdout.write('\t')
					sys.stdout.write("{0:2.2f}".format(DNA_RNA_Tm(sequences_targets_rev[seq_key]["aligned_matches"][targ_id]["target_seq"])))
					
					if "exon_number" in sequences_targets_rev[seq_key]["orig_matches"][targ_id].keys():
						sys.stdout.write('%20s' %sequences_targets_rev[seq_key]["orig_matches"][targ_id]["exon_number"])

					if (pam_seq == "NGG") and (length == 20):
						if "seq_forscore" in sequences_targets_rev[seq_key]["aligned_matches"][targ_id]:
							sys.stdout.write('\t')
							sys.stdout.write("{0:3.2f}".format(calcDoenchScore(sequences_targets_rev[seq_key]["aligned_matches"][targ_id]["seq_forscore"])))
						else:
							sys.stdout.write('\t')
							sys.stdout.write('%5s' % "")
		...
		for targ_id in seq_matches[seq_id]["direct_targets"].keys():
			...
			if unique:
				...
				if mol_type != "refseq": 
					...
					if seq_id in seqs_exons:
						# check if this target does not overlap two exons			
						for exonID in sorted(coords_exons[seq_id], key = lambda key: coords_exons[seq_id][key]["start"]):
							if target_seq in seqs_exons[seq_id][exonID]:
								inside = True
	...
	# for	seq in unique_targets.keys():
		...
		# if len(unique_targets[seq]) > 0:
			...
			# for target in unique_targets[seq].keys():
				...
				# # check if this target does not overlap two exons			
				# for exonID in seqs_exons[seq]["exons"].keys():
					
					# if target_seq in seqs_exons[id][exonID]["exon_seq"]:
						# inside = True
		...
		if len(unique_targets[seq_id]) == 0:
			...
			for targ_id in unique_targets[seq_id].keys():
				...
				if unique_targets[seq_id][targ_id]["orientation"] == 'forward':
					...
					# add exon number to a target if this information is available
					#if unique_targets[seq_id][targ_id]["exon_number"]:
					#	unique_targets_sequences[seq_id]["direct_targets"][targ_id]["exon_number"] = unique_targets[seq_id][targ_id]["exon_number"]					
					...
					# add exon number to a target if this information is available
					#if unique_targets[seq_id][targ_id]["exon_number"]:	
					#	unique_targets_sequences[seq_id]["reverse_targets"][targ_id]["exon_number"] = unique_targets[seq_id][targ_id]["exon_number"]	
...
# the value "different" means that different identifiers are from different species
species = form["species"].value

# retrieve the molecule type
mol_type = form["moltype"].value

# obtain the input gene identifiers from the user
geneids_input = form["input_seq"].value
...
# case 1: the sequence identifiers field was submitted empty
if geneids_input == "":
# generate an error	
	badrequest_seq(geneids_input)
...
# Break the program execution if there are fewer than 2 identifiers
if id_count < 2:
	badrequest_seq(geneids)
...
for gene_id in geneids:
	...
	# assume that the gene identifier is a gene symbol
		...
		#	badrequest_seq(gene_id)
		#else: 		
		#	# deal with the absence of exon boundary checking downstream in the code
		...
		Entrez.email = "s.prykhozhij@gmail.com"
		...
		if len(rec) != 0:
			...
			for seq_record in SeqIO.parse(handle, "gb"):
				genes_transcripts[orig_id] = str(seq_record.seq)
...
#    computationally expensive
		...
		for record in align:
			...
			if exon_N:
				sequences_matches_forward[id]["orig_matches"][match_count]["exon_number"] = exon_N
			exon_N = ""
...
for match in regex.finditer(rev_align_cons["consensus"]):
	...
	if match.group(1).count('X') <= 1:	# to exclude the sequences which contain more than 1 non-consensus site
		...
		for record in align:
			...
			if exon_N:
				sequences_matches_reverse[id]["orig_matches"][match_count]["exon_number"] = exon_N
			exon_N = ""
